...
function _source_setup() {
    ...
    if [[ "$*" =~ '--local' ]]; then
        ...
        git clone https://github.com/deadmanpty/swizzin-stickz/swizzin.git /etc/swizzin >> ${log} 2>&1
...
_os() {
    ...
    if [[ ! $codename =~ ^(focal|bullseye|jammy|bookworm|noble|trixie|resolute)$ ]]; then
